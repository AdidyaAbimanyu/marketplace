            ...
            <div class="col-md-8">
                <div class="border p-3">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="address" id="address" rows="10" placeholder="Masukkan alamat lengkap" name="alamat" required></textarea>
                    </div>
                    <h5 class="mt-4">Products</h5>
                    <ul class="list-group">
                        @foreach ($cartItems as $index => $item)
                            <li class="list-group-item d-flex justify-content-between align-items-center">
                                {{ $item->produk->nama_produk ?? 'Produk tidak ditemukan' }} (x {{ $item->jumlah_produk }})
                                <span class="badge bg-primary rounded-pill">Rp. {{ number_format($item->total_harga, 0, ',', '.') }}</span>
                            </li>

                            <!-- Hidden inputs for product ID and quantity -->
                            <input type="hidden" name="items[{{ $index }}][id_produk]" value="{{ $item->id_produk }}">
                            <input type="hidden" name="items[{{ $index }}][quantity]" value="{{ $item->jumlah_produk }}">
                        @endforeach
                    </ul>
                </div>
            </div>
            ...
            <div class="col-md-4">
                <div class="border p-4">
                    ...
                    <table class="table borderless">
                        ...
                        <tr>
                            <td>Discount</td>
                            <td class="text-end text-success">Rp.{{ number_format($data['discount'], 0, ',', '.') }}</td>
                        </tr>
                        <tr>
                            <td>Tax</td>
